...
class _LiveScreenPushPageState extends State<LiveScreenPushPage> {
  ...
  startPush() async {
    ...
    var sharedCode = await _livePusher
        ?.startScreenCapture("group.com.tencent.liteav.RPLiveStreamShare");
    if (sharedCode == null || sharedCode != V2TXLIVE_OK) {
      showErrordDialog(
          "Start screen share error: Please check Control Center Screen Recording");
      return;
    }
    ...
    if (pushCode == null || pushCode != V2TXLIVE_OK) {
      ...
      _livePusher?.stopScreenCapture();
      showErrordDialog(
          "Start screen share error:: push error($pushCode)，Please check push url or license");
      return;
    }
